...
<el-input placeholder="任务id" v-model="params.taskid" style="width: 80px;" class="filter-item"/>
<el-select v-model="params_taskstatus" placeholder="任务状态" clearable style="width: 120px" class="filter-item">
        <el-option v-for="item in taskstatusOptions" :key="item.label" :label="item.label" :value="item.value" />
      </el-select>
...
<script>
  ...
  import {getNewTaskList} from '@/api/log'
...
pagesizeOptions: [{
    ...
    debugger
    params.taskstatus = this.params_taskstatus
        return new Promise((resolve, reject) => {
          getNewTaskList(curPage, params).then(response => {
      ...
      handleSearch() {
        ...
        exportExcel() {        
         import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['记录时间', '角色ID', '角色名称', '服务器','用户ID', '帐号', '平台', '操作系统',  '任务类型ID', '任务ID','任务状态','战斗']
           const filterVal = ['logtime', 'roleid', 'rolename', 'serverid','userid', 'account', 'params3','params6', 'params9', 'params5', 'params1','params4']

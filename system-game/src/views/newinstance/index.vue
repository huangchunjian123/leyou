...
<el-input placeholder="副本id" v-model="params.instanceid" style="width: 120px;" class="filter-item"/>
<el-select v-model="params_instancestatus" placeholder="副本状态" clearable style="width: 120px" class="filter-item">
        <el-option v-for="item in instancestatusOptions" :key="item.label" :label="item.label" :value="item.value" />
      </el-select>
...
<script>
  ...
  import {getNewInstanceList} from '@/api/log'
...
params.instanceid= this.params.instanceid
  debugger
params.instancestatus= this.params_instancestatus
        return new Promise((resolve, reject) => {
          getNewInstanceList(curPage, params).then(response => {
      ...
      handleSearch() {
        ...
        exportExcel() {        
         import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['记录时间', '角色ID', '角色名称', '服务器','用户ID', '帐号', '平台', '操作系统',  '章节ID', '副本ID','副本状态','战斗']
           const filterVal = ['logtime', 'roleid', 'rolename', 'serverid','userid', 'account', 'params7','params3', 'params9', 'params5', 'params1','params6']

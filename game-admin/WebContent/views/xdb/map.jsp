...
$(document).ready(function () {
	var surl= '';
	var columns = [];
   	surl = escape('<%=request.getParameter("surl")%>');
    $.ajax({
    url: '<%=request.getContextPath()%>/xdbMapData/getmapcols?fieldnames=<%=request.getParameter("fieldnames")%>'+'&surl='+surl,
        type: 'GET',
       async: false,
        dataType: 'json',
   	...
   	function showMapMapDailog(index,field){
  		...
  		$("#dd").dialog({
	        ...
	        onLoadSuccess:function(data){    
	      	...
	      	,
		href:'<%=request.getContextPath()%>/views/xdb/map.jsp?surl='+surl+'&fieldnames='+field+'&fieldkeys='+fieldkey,
		});
  		fieldnames = field ;
